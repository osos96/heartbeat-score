...
// GeoJSON district names that map to each Bosta zone
...
// Box-Muller Gaussian via seeded uniform
const gaussianPoint = (lat, lng, sigma, seed) => {
  const u1 = Math.max(1e-10, srand(seed))
  const u2 = srand(seed + 1000)
  const mag = sigma * Math.sqrt(-2 * Math.log(u1))
  return [
    lat + mag * Math.cos(2 * Math.PI * u2),
    lng + mag * Math.sin(2 * Math.PI * u2),
  ]
}

// Generate dense thermal-camera-style heatmap points for a hub
// Returns [lat, lng, intensity] triples — hundreds of points in tight clusters
const hubHeatPoints = (hubId, score) => {
  const [hubLat, hubLng] = HUB_COORDS[hubId] || [30, 31]
  const pts = []
  const hubSeed = hubId.split('').reduce((s, c) => s * 31 + c.charCodeAt(0), 7)

  // 4-6 primary hotspot cluster centers (simulating busy streets/buildings)
  const numClusters = 4 + (hubSeed % 3)
  const clusterCenters = []
  for (let ci = 0; ci < numClusters; ci++) {
    const s1 = hubSeed * 17 + ci * 137
    const r = 0.004 + srand(s1) * 0.012          // 400m – 1.6km from hub
    const a = srand(s1 + 500) * Math.PI * 2
    clusterCenters.push([
      hubLat + r * Math.cos(a),
      hubLng + r * Math.sin(a),
    ])
  ...
  // Hub itself is always a hotspot cluster center
  clusterCenters.push([hubLat, hubLng])

  // Generate dense points around each cluster (tight Gaussian)
  const intensity = 0.55 + (score / 100) * 0.45
  clusterCenters.forEach(([cLat, cLng], ci) => {
    const clusterCount = 50 + Math.round(srand(hubSeed + ci * 41) * 60)  // 50-110 pts
    for (let i = 0; i < clusterCount; i++) {
      const seed = hubSeed * 3 + ci * 211 + i * 7
      // Tight core: sigma ~0.0018° ≈ 200m
      const [plat, plng] = gaussianPoint(cLat, cLng, 0.0018, seed)
      const dist = Math.hypot(plat - cLat, plng - cLng)
      const w = Math.max(0.1, intensity * Math.exp(-dist * dist / (2 * 0.002 * 0.002)))
      pts.push([plat, plng, w])
    }
    // Halo ring around each cluster (shows reach boundary)
    for (let i = 0; i < 20; i++) {
      const seed = hubSeed * 5 + ci * 311 + i * 13
      const [plat, plng] = gaussianPoint(cLat, cLng, 0.005, seed)
      pts.push([plat, plng, intensity * 0.25])
    }
  })

  // Background scatter across hub service area (sparse — cool areas)
  for (let i = 0; i < 80; i++) {
    const seed = hubSeed * 7 + i * 19
    const r = 0.002 + srand(seed) * 0.018
    const a = srand(seed + 200) * Math.PI * 2
...
// KPI trend badge: ▲/▼ with percentage delta
...
// ── GeoJSON-powered Leaflet map ──────────────────────────────────────────────
const LeafletMap = ({ cities, onSelectCity, expandedZone, onHubSelect, activeHubId }) => {
  const containerRef = useRef(null)
  const mapRef       = useRef(null)
  const layersRef    = useRef([])
  const heatRef      = useRef(null)
  const geoCitiesRef = useRef(null)   // admin1 geojson cache
  const geoDistRef   = useRef(null)   // admin2 geojson cache
  ...
  const getCityScore = useCallback((govLabel) => {
    const city = cities.find(c => c.name === govLabel)
    return city ? city.scHB : null
  }, [cities])

  const getZoneScore = useCallback((zoneName) => {
    for (const city of cities) {
      const zone = city.zones?.find(z => z.name === zoneName)
      if (zone) return zone.scHB
    }
    return null
  }, [cities])

  // Render country-level: admin1 polygons coloured by HeartBeat score
  const renderCountry = useCallback(async (L) => {
    ...
    const layer = L.geoJSON(geo, {
      style: feat => {
        ...
        const score = getCityScore(label)
  ...
  // Render city-level: admin2 districts grouped by zone colour + hub pins
  const renderCity = useCallback(async (L, cityLabel) => {
    ...
    // Build zone-colour lookup
    const zoneColorMap = {}
    for (const city of cities) {
      city.zones?.forEach(zone => {
        const score = zone.scHB || 0
        zoneColorMap[zone.name] = hbColor(score)
      })
    }

    // Determine gov name from label mapping
    ...
    // Find zone for a district name
    const distToZone = {}
    Object.entries(ZONE_TO_DISTRICTS).forEach(([zone, info]) => {
      if (info.gov === gov) {
        info.names.forEach(d => { distToZone[d] = zone })
      }
    })
    ...
    // Zone label markers at zone centroid
    const zoneCentroidMap = {}
    for (const city of cities) {
      city.zones?.forEach(zone => {
        const info = ZONE_TO_DISTRICTS[zone.name]
        if (!info || info.gov !== gov) return
        zoneCentroidMap[zone.name] = zone
      })
    }
        ...
        zone.hubsList?.forEach(hub => {
          ...
          const hIcon = L.divIcon({
            html: `<div style="
              ...
              border-radius:50%;background:${hcol};
              border:${isActive ? '3px' : '2px'} solid white;
              box-shadow:0 1px ${isActive ? 8 : 4}px rgba(0,0,0,${isActive ? 0.4 : 0.25});
              ${isActive ? `ring:2px solid ${hcol};` : ''}
            "></div>`,
            className: '', iconAnchor: [isActive ? 7 : 5, isActive ? 7 : 5],
          })
          const m = L.marker(coords, { icon: hIcon })
            .addTo(mapRef.current)
  ...
  // Build and render the heatmap for a zone (or hub-filtered subset)
  const renderHeat = useCallback((L, zoneName, filterHubId) => {
    ...
    for (const city of cities) {
      for (const zone of (city.zones || [])) {
        ...
        for (const hub of (zone.hubsList || [])) {
          if (filterHubId && hub.id !== filterHubId) continue
          allPoints = allPoints.concat(hubHeatPoints(hub.id, hub.scHB || 80))
        }
      }
    }
    ...
    if (window.L.heatLayer && allPoints.length) {
      heatRef.current = window.L.heatLayer(allPoints, {
        radius:   14,   // tight radius → granular texture
        blur:     10,   // low blur → sharp hotspot boundaries
        maxZoom:  17,
        max:      1.0,
        minOpacity: 0.25,
  ...
  // Sync country/city view
  useEffect(() => {
    if (!ready || !mapRef.current) return
    const L = window.L
    if (drillCity) {
      renderCity(L, drillCity)
    } else {
      renderCountry(L)
    }
  }, [ready, cities, drillCity, renderCity, renderCountry])

  // Zoom to zone + draw heatmap
  useEffect(() => {
    if (!ready || !mapRef.current || !expandedZone || !drillCity) return
    const bounds = ZONE_BOUNDS[expandedZone]
    if (bounds) { try { mapRef.current.fitBounds(bounds, { padding:[30,30] }) } catch(e){} }
    renderHeat(window.L, expandedZone, activeHubId)
  }, [ready, expandedZone, drillCity, cities, renderHeat, activeHubId])

  // Re-render heatmap when hub filter changes
  useEffect(() => {
    ...
    <div style={{ position:'relative', border:`1px solid ${T.border}`, borderRadius:12, overflow:'hidden', height:340 }}>
      ...
      {activeHubId && expandedZone && (
        <div style={{
          ...
          background:'rgba(37,99,235,0.9)', border:'none', borderRadius:7,
          padding:'5px 12px', fontSize:11, fontWeight:700, color:'#fff',
          boxShadow:T.shadow,
        }}>Hub filter active — click hub again to reset</div>
      )}
      ...
      <style>{`
        .city-tooltip {
          ...
          font-family: Inter, sans-serif; padding: 3px 8px;
...
export default function Dashboard() {
  ...
  const [activeMapHub, setActiveMapHub] = useState(null)  // hub filter for heatmap
  ...
  // Previous period lookup maps for trend badges
  ...
  const handleHubSelect = useCallback(hubId => {
    setActiveMapHub(hubId)
  }, [])
  ...
  return (
    <div style={{ padding: '100px 2rem 60px', maxWidth: 1300, margin: '0 auto' }}>
      ...
      {/* Drill table — same structure, hubs always visible, KPI trend badges added */}
      <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 12, overflow: 'hidden', boxShadow: T.shadow }}>
        ...
        {cities.map(city => {
          ...
          const pCity = prevCityMap[city.name] || {}
          return (
            <React.Fragment key={city.name}>
              <div onClick={() => {
                ...
                <div style={{ fontWeight: 800, fontSize: 15, color: hbColor(city.scHB||0) }}>
                  {(city.scHB||0).toFixed(1)}%
                  <TrendBadge curr={city.scHB||0} prev={pCity.scHB}/>
                </div>
                <div style={{ fontSize: 13, color: hbColor(city.scStars||0) }}>
                  {(city.scStars||0).toFixed(1)}%
                  <TrendBadge curr={city.scStars||0} prev={pCity.scStars}/>
                </div>
                <div style={{ fontSize: 13, color: hbColor(city.scHubs||0) }}>
                  {(city.scHubs||0).toFixed(1)}%
                  <TrendBadge curr={city.scHubs||0} prev={pCity.scHubs}/>
                </div>
                <div style={{ fontSize: 13, color: hbColor(city.scMerch||0) }}>
                  {(city.scMerch||0).toFixed(1)}%
                  <TrendBadge curr={city.scMerch||0} prev={pCity.scMerch}/>
                </div>
              </div>

              {ce && city.zones?.map(zone => {
                const ze    = expandedZone === zone.name
                const pZone = prevCityMap[`z:${zone.name}`] || {}
                return (
                  <React.Fragment key={zone.name}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 130px 110px 110px 110px', gap: 8,
                      ...
                      <div style={{ fontWeight: 700, fontSize: 13, color: hbColor(zone.scHB||0) }}>
                        {(zone.scHB||0).toFixed(1)}%
                        <TrendBadge curr={zone.scHB||0} prev={pZone.scHB}/>
                      </div>
                      <div style={{ fontSize: 12, color: hbColor(zone.scStars||0) }}>
                        {(zone.scStars||0).toFixed(1)}%
                        <TrendBadge curr={zone.scStars||0} prev={pZone.scStars}/>
                      </div>
                      <div style={{ fontSize: 12, color: hbColor(zone.scHubs||0) }}>
                        {(zone.scHubs||0).toFixed(1)}%
                        <TrendBadge curr={zone.scHubs||0} prev={pZone.scHubs}/>
                      </div>
                      <div style={{ fontSize: 12, color: hbColor(zone.scMerch||0) }}>
                        {(zone.scMerch||0).toFixed(1)}%
                        <TrendBadge curr={zone.scMerch||0} prev={pZone.scMerch}/>
                      </div>
                    </div>
                    ...
                    {/* Hubs always visible when zone is expanded */}
                    {ze && zone.hubsList?.map(hub => {
                      const he    = expandedHub === hub.id
                      const pHub  = prevCityMap[`h:${hub.id}`] || {}
                        ...
                        <React.Fragment key={hub.id}>
                          <div onClick={() => setExpandedHub(he ? null : hub.id)}
                            ...
                            <div style={{ fontWeight: 700, fontSize: 13, color: hbColor(hub.scHB||0) }}>
                              {(hub.scHB||0).toFixed(1)}%
                              <TrendBadge curr={hub.scHB||0} prev={pHub.scHB}/>
                            </div>
                            <div style={{ fontSize: 12, color: hbColor(hub.scStars||0) }}>
                              {(hub.scStars||0).toFixed(1)}%
                              <TrendBadge curr={hub.scStars||0} prev={pHub.scStars}/>
                            </div>
                            <div style={{ fontSize: 12, color: hbColor(hub.scHubs||0) }}>
                              {(hub.scHubs||0).toFixed(1)}%
                              <TrendBadge curr={hub.scHubs||0} prev={pHub.scHubs}/>
                            </div>
                            <div style={{ fontSize: 12, color: hbColor(hub.scMerch||0) }}>
                              {(hub.scMerch||0).toFixed(1)}%
                              <TrendBadge curr={hub.scMerch||0} prev={pHub.scMerch}/>
                            </div>
                          </div>
